...
import { LayoutDashboard, Package, ArrowRightLeft, Menu, Box, LogOut, Command, Bell, Share, X } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isSupabaseConfigured) {
      supabase.auth.getSession().then(({ data: { session } }) => {
        if (session?.user) {
          setUser({
            id: safeString(session.user.id),
            email: safeString(session.user.email),
            name: safeString(session.user.email?.split('@')[0] || 'User'),
            role: 'admin'
          });
        }
      });
  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB] overflow-hidden text-gray-900 font-sans selection:bg-brand-100 selection:text-brand-900 relative">
      ...
      {showIosInstall && (
        <div className="fixed bottom-4 left-4 right-4 z-50 bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-gray-100 animate-in slide-in-from-bottom duration-500">
          <div className="flex items-start gap-4">
            ...
            <button onClick={() => setShowIosInstall(false)} className="p-1 text-gray-400 hover:text-gray-900">
              <X size={20} />
            </button>
          </div>
        </div>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-gray-900/20 z-20 lg:hidden backdrop-blur-sm transition-opacity" onClick={() => setSidebarOpen(false)} />
      )}

      <aside className={`fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-gray-100 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="px-3 py-4 space-y-1">
          {navItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${currentView === item.id ? 'bg-gray-50 text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
            >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white/80 backdrop-blur-sm border-b border-gray-100 h-16 flex items-center justify-between px-6 lg:px-8 z-10 sticky top-0">
          <div className="flex items-center gap-4">
             <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 text-gray-500 hover:bg-gray-100 rounded-md lg:hidden">
              <Menu size={24} />
            </button>
            <h1 className="text-lg font-semibold text-gray-900 capitalize tracking-tight">{safeString(currentView)}</h1>
          </div>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-8">
          <div className="max-w-6xl mx-auto space-y-8">
            {currentView === 'dashboard' && (
              <Dashboard inventory={inventory} transactions={transactions} userRole={user.role} onNavigate={(view) => setCurrentView(view)} />
            )}
            {currentView === 'inventory' && (
              <Inventory inventory={inventory} userId={user.id} onRefresh={fetchData} />
            )}
            {currentView === 'transactions' && (
              <Transactions transactions={transactions} products={products} inventory={inventory} userRole={user.role} userId={user.id} onRefresh={fetchData} />
            )}
          </div>
        </main>
      </div>
